...
void FlowControl::Update(float dt_s, float rtt) {
    const float RTT_threshold = 250.0f;
    if (mode_ == Good) {
        if (rtt > RTT_threshold) {
            LOGI("[FlowControl]: dropping to bad mode");
            mode_ = Bad;
            if (good_conditions_time_ < 10.0f && penalty_time_ < 60.0f) {
                ...
                LOGI("[FlowControl]: penalty time increased to %f", penalty_time_);
            }
            good_conditions_time_ = 0.0f;
            penalty_reduction_acc_ = 0.0f;
        } else {
            ...
            if (penalty_reduction_acc_ > 10.0f && penalty_time_ > 1.0f) {
                ...
                LOGI("[FlowControl]: penalty time reduced to %f", penalty_time_);
        ...
        if (good_conditions_time_ > penalty_time_) {
            LOGI("[FlowControl]: upgrading to good mode");
            good_conditions_time_ = 0.0f;
            penalty_reduction_acc_ = 0.0f;
            mode_ = Good;
        }
